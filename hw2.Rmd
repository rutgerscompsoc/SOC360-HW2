...
### Set-up
You will need to have a Spotify account and an active Spotify API key for this part of the homework. You can use the same credentials you created for the in-class assignment. If you do not have credentials, please do the following:
...
d. Calculate the proportion of explicit tracks in each playlist. Which playlist has the highest proportion of explicit tracks? Which has the lowest?
...
d. We have not covered the `melt` function in class, but it can be useful for transforming data. Explain what the function does and why it is used here. You may want to look at the documentation and examine the objects below and after this transformation.
Answer:
...
Complete the code below to extract the two tables from the page and store them as separate objects. Note that there is a specific function in `rvest` which can help you to process the tables. I have provided the relevant CSS selector for the table. Hint: `rverst` includes a function specifically designed for extracting tables from HTML.
...
b. Complete the regular expression in `str_extract` to detect whether there is a numeric year (e.g. 1848, 1968, 2011) in each element. Note: You do not need to detect all years, since some rows may have multiple years (e.g. 1501-1504), any reference is fine. You may want to look up the `strtoi` function I have provided to see how it works.

c. Combine `bullets` and `years` into a two column tibble called `events` then drop any rows where no valid year was detected. 
